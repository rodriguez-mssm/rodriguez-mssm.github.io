...
function field(label, name, initial, options = {}) {
  ...
  const attrs = [options.step && `step="${options.step}"`, options.min != null && `min="${options.min}"`, options.max != null && `max="${options.max}"`, options.maxlength && `maxlength="${options.maxlength}"`, options.required && "required"].filter(Boolean).join(" ");
    ...
    form.onsubmit = async (event) => {
      ...
      try {
        ...
        const payload = {
          ...
          sample_location: form.sampleLocation.value.trim() || null,
